...
The workspace-management capability creates and manages isolated, per-issue working
directories rooted at the configured workspace root. It materializes the configured
repositories, initializes the `.conductor/` skeleton, executes lifecycle hooks with
timeout enforcement and audit logging, enforces filesystem safety invariants, runs agents
as workspace-scoped subprocesses, and classifies failures using the SPEC §23.4 error
identifiers.
