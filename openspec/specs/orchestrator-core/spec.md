...
The orchestrator-core capability is the central control loop of Conductor. It owns the
single authoritative in-memory runtime state, models each issue execution as a Run
Attempt, and runs a poll loop that reconciles active runs, selects and sorts dispatch
