...
- [internal/tools/](internal/tools/) — Conductor tool injection: Tool/Registry/Dispatcher, the eight built-in tools, approval gating, credential isolation, ToolPlugin
...
- Phase 13 Conductor Tool Injection (Tool/Registry/Dispatcher model with consumer-side engine interfaces, the eight SPEC §7.3 built-in tools, provider tool-result continuation `ContinueWithToolResults` for Anthropic + the shared OpenAI-compatible path, router-driven tool-call dispatch loop bounded by a tool-turn cap, `approval_policy` enforcement with honest approval-required denial, credential isolation with audit redaction, `ToolPlugin` registration, and the GitHub GC `TrackerIssuer` wiring) → [internal/tools/](internal/tools/), [internal/provider/](internal/provider/) (`ContinueWithToolResults`), [internal/router/](internal/router/) (dispatch loop), [cmd/conductor/cmd/](cmd/conductor/cmd/) (`tools.go`, `gcissuer.go`)
...
- Phase 17 Container Isolation & Hardening (opt-in `workspace.container` config, `ContainerRunner` + Docker runner behind an injected client with the live client build-tagged `docker_live`, workspace-mounted/air-gapped/resource-limited container path selected by the workspace Manager — the default subprocess `AgentCommand` path is unchanged when no container is configured, host tool Unix-socket transport, audit secret redaction at the writer boundary, Profile C `Dockerfile` + sample Helm chart/manifests under `deploy/`) → [internal/workspace/](internal/workspace/), [internal/audit/](internal/audit/), [deploy/](deploy/)
